<div class="bottom-panel d-flex align-items-center">
    ...
    <button><a href="<?= linkTo("reply/show/{$reply->id}"); ?>"><i class="fas fa-plus-square me-1"></i>Expand</a></button>
    <?php require component("reply-vote"); ?>
...
<form action="<?= linkTo("reply/create/{$reply->id}"); ?>" method="POST" class="mt-4 d-none" id="<?= htmlspecialchars($reply->id); ?>">
...
<div class="mt-3">
    ...
    <?php else : ?>
        <?php if ($reply->replies()) : ?>
            <a class="continue-thread" href="<?= linkTo("reply/show/{$reply->id}"); ?>">Continue Thread...</a>
        <?php endif; ?>
    <?php endif; ?>
